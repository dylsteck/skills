...
1. **Analyze project**: Run `python scripts/analyze_project.py <project_dir>` to find all MiniKit usage
...
7. **Validate**: Run `python scripts/validate_conversion.py <project_dir>`

## Key Differences

- **No provider needed**: Farcaster SDK uses direct imports, not React context
- **All actions are async**: Always use `await sdk.actions.*()` 
- **Direct context access**: Use `sdk.context` directly, not through a hook
